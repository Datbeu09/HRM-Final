...
export default function FinancePayrollItem({
  ...
  const hasQuickView = typeof onQuickView === "function";
  ...
  return (
    <div
      className={`
        ...
        ${isDone ? "bg-slate-50/60" : "bg-white hover:bg-slate-50 hover:shadow-sm hover:border-primary/30"}
      `}
    >
      ...
      {!isDone ? (
        <div className="text-left sm:text-right">
          ...
          <div className="mt-2 flex gap-2 sm:justify-end">
            <button
              type="button"
              onClick={hasQuickView ? onQuickView : undefined}
              disabled={!hasQuickView}
              className={`
                inline-flex items-center gap-2 px-3 h-9 rounded-xl
                border border-border bg-white text-slate-700 text-xs font-semibold transition
                ${hasQuickView ? "hover:bg-slate-50" : "opacity-60 cursor-not-allowed"}
              `}
            >
