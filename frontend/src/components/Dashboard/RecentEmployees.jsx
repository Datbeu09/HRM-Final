...
export default function RecentEmployees() {
  ...
  const loadEmployees = useCallback(() => {
    ...
    getEmployees()
      .then((data) => {
        ...
        const recentEmployees = list.filter((e) => isWithinLastMonth(e.startDate));

        const next = recentEmployees.slice(0, page * 5);
        setEmployees(next);
  ...
  const handleScroll = (e) => {
    const el = e.target;
    const bottom = el.scrollTop + el.clientHeight >= el.scrollHeight - 2; // tránh lệch px
